...
```{r}
data_eda = read_csv("spreadspoke_scores.csv", 
...
rpart.fit <- train(dif ~ . ,
                   data = train_data,
                   method = "rpart",
                   tuneGrid = data.frame(cp = exp(seq(-50,-1, length = 100))),
                   trControl = ctrl)
...
rmse_rf <- RMSE(rf_prediction, test_resp)
rmse_rf
```
...
rmse_rf_train <- RMSE(rf_prediction_train, train_resp)
rmse_rf_train
```
